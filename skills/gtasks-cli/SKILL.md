...
All task commands can optionally specify a task list using the `-l` flag. For this Codex environment, use `My Tasks` as the default task list whenever the user does not specify one, and pass `-l "My Tasks"` for automated commands to avoid interactive prompts. If omitted, gtasks uses `GTASKS_DEFAULT_TASKLIST` env var or `tasks.default_task_list` from the config file. If only one list exists, it is selected automatically. Otherwise you'll be prompted interactively.
...
2. **Use task list flag for automation**: When scripting, use `-l "My Tasks"` unless the user specifies a different list name. This avoids interactive prompts.
...
- Always use `My Tasks` when the user does not specify a task list
